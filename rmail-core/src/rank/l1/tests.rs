...
/// Task 33's `Explain` reads [`Weights::contributions`]/[`L1Ranker::contributions`]
/// to show a per-feature breakdown — this pins the property that makes such a
/// breakdown trustworthy: summing every reported `weighted_contribution` must
/// reproduce exactly the score [`L1Ranker::rank`]/[`Weights::score`] actually
/// produced for the identical `(features, intent)` pair. A breakdown that
/// does not reconcile with the real score is worse than none — see this
/// module's own `contributions` doc comment.
#[test]
fn contributions_sum_to_the_score_the_ranker_actually_produced() {
    let weights = Weights::cold_start();
    let ranker = L1Ranker::new(weights.clone());
    let v = formula_vector();

    for intent in [Intent::Navigational, Intent::Exploratory, Intent::Lookup] {
        let score = weights.score(&v, intent);
        let contributions = weights.contributions(&v, intent);

        // Every `FeatureVector::as_pairs` entry is represented, in order.
        assert_eq!(contributions.len(), v.as_pairs().len());

        let summed: f64 = contributions
            .iter()
            .map(|(_, _, _, weighted)| weighted)
            .sum();
        approx_eq(summed, score);

        // `L1Ranker::contributions` is the same decomposition, reached
        // through the ranker rather than the raw `Weights` table.
        let via_ranker = ranker.contributions(&v, intent);
        assert_eq!(via_ranker, contributions);
        let via_ranker_score: f64 = via_ranker.iter().map(|(_, _, _, w)| w).sum();
        approx_eq(via_ranker_score, ranker.score(&v, intent));
    }
}

/// The intent gate ([`bulk_downweight_suppressed`]) has to be visible in the
/// *contribution*, not just the aggregate score: an `is_newsletter`/
/// `is_automated` feature reads `weight == 0.0` (and therefore
/// `weighted_contribution == 0.0`) under an intent that suppresses it, even
/// though [`Weights::get`] still holds the configured (nonzero) raw weight —
/// showing the raw weight instead would tell an `--explain` reader "this
/// penalized the result" when it provably did not.
#[test]
fn a_suppressed_bulk_downweight_contributes_zero_not_the_raw_weight() {
    let weights = Weights::cold_start();
    let mut v = zero_vector();
    v.is_newsletter = true;
    v.is_automated = true;

    // Navigational suppresses both terms entirely (see
    // `bulk_downweight_suppressed`'s doc comment).
    let nav = weights.contributions(&v, Intent::Navigational);
    let newsletter = nav
        .iter()
        .find(|(name, ..)| *name == FeatureName::IsNewsletter)
        .expect("is_newsletter is always present");
    let automated = nav
        .iter()
        .find(|(name, ..)| *name == FeatureName::IsAutomated)
        .expect("is_automated is always present");
    approx_eq(newsletter.2, 0.0); // weight
    approx_eq(newsletter.3, 0.0); // weighted_contribution
    approx_eq(automated.2, 0.0);
    approx_eq(automated.3, 0.0);
    // The raw, ungated weight is still nonzero — proving the zero above is
    // the suppression firing, not a cold-start table with no penalty at all.
    assert!(weights.get(FeatureName::IsNewsletter) < 0.0);
    assert!(weights.get(FeatureName::IsAutomated) < 0.0);

    // Exploratory suppresses neither: both contribute their full configured
    // (negative) weight.
    let exploratory = weights.contributions(&v, Intent::Exploratory);
    let newsletter = exploratory
        .iter()
        .find(|(name, ..)| *name == FeatureName::IsNewsletter)
        .expect("is_newsletter is always present");
    approx_eq(newsletter.2, weights.get(FeatureName::IsNewsletter));
    approx_eq(newsletter.3, weights.get(FeatureName::IsNewsletter) * 1.0);
}
